		...
		var chart = new CanvasJS.Chart("chartContainer",
		{
			...
			{        
				...
				<?php
					...
					$result = mysql_query("SELECT `COUNT`, `DATE` FROM `keywords` WHERE `CATEGORY` = '蔡英文' and `DATE` = '$day[$i]'");
			...
			{        
				...
				<?php
					for($i = 0; $i < sizeof($day); $i++){
					$result = mysql_query("SELECT `COUNT`, `DATE` FROM `keywords` WHERE `CATEGORY` = '朱立倫' and `DATE` = '$day[$i]'");
					$num_rows = mysql_num_rows($result);
					$date = explode('-', $day[$i]);
					if($num_rows == 0){
			...
			{        
				...
				<?php
					for($i = 0; $i < sizeof($day); $i++){
					$result = mysql_query("SELECT `COUNT`, `DATE` FROM `keywords` WHERE `CATEGORY` = '宋楚瑜' and `DATE` = '$day[$i]'");
					$num_rows = mysql_num_rows($result);
					$date = explode('-', $day[$i]);
					if($num_rows == 0){
